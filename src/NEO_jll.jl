...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import NEO_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "NEO_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"NEO", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "NEO_jll", "src", "NEO_jll.jl"))
        @info("NEO_ll dev'ed out to /depot/dev/NEO_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("700fe977-ac61-5f37-bbc8-c6c4b2b6a9fd"))
platforms = [Pkg.Artifacts.unpack_platform(e, "NEO", artifacts_toml) for e in artifacts["NEO"]]
...
if best_platform === nothing
    @debug("Unable to load NEO; unsupported platform $(triplet(platform_key_abi()))")
else
